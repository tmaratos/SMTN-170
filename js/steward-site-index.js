...
(function initStewardSiteIndex(global) {
  ...
  const PAGE_REGISTRY = {
    ...
    "calendar.html": {
      ...
      path: "calendar.html",
      section: "Main",
      summary: "Read-only month view of every approved squadron event. Browse the grid, jump to today, and filter by category. Edit events in Tasks or the Monthly Meeting Schedule Builder.",
      primaryActions: ["View this month", "Jump to today", "Filter event categories"],
      forms: [],
      buttons: ["Previous month", "Next month", "Today", "Meeting planner"],
      relatedPages: ["schedule.html", "tasks.html", "flight-review.html"],
      keywords: ["calendar", "events", "meetings", "schedule", "month", "grid", "view"],
    },
    "schedule.html": {
      pageId: "schedule",
      title: "Meetings",
      path: "schedule.html",
      section: "Main",
      summary: "Build the monthly squadron meeting schedule with uniforms, training nights, and weekly blocks; clone previous month; print and export PDF.",
      primaryActions: [
      ...
      relatedPages: ["calendar.html", "documents.html", "schedule-print.html"],
      keywords: [
        "meetings",
        "schedule",
        "meeting planner",
        ...
        "preview print schedule",
        "saved monthly schedules",
        "saved schedules",
      ],
    },
    "schedule-print.html": {
      pageId: "schedule-print",
      title: "Monthly Meeting Schedule (Print View)",
      path: "schedule-print.html",
      section: "Main",
      summary: "Standalone print/PDF document view of the monthly squadron meeting schedule.",
      primaryActions: ["Print", "Save as PDF"],
      forms: [],
      buttons: ["Print / Save as PDF", "Close"],
      relatedPages: ["schedule.html"],
      keywords: [
        "print schedule",
        "monthly schedule pdf",
    ...
    "orgchart.html": {
      ...
      path: "orgchart.html",
      section: "Main",
      summary: "Build and update the squadron Table of Organization report — commander, primary staff row, cadet branch, and custom positions; print and export PDF.",
      primaryActions: [
        "Edit Org Chart",
        "Reset to TN-170 Default Structure",
        "Save Org Chart",
        "Clone Existing Chart",
        "Preview Print Layout",
        "Print Org Chart",
        "Export Org Chart PDF",
        "Ask Steward",
      ],
      forms: ["orgChartBuilderForm"],
      buttons: [
        "Save Org Chart",
        "Reset to TN-170 Default Structure",
        "Clone Existing Chart",
      ...
      relatedPages: ["admin.html", "senior-member.html", "orgchart-print.html"],
      keywords: [
        "org chart",
        "organization",
        "organization chart",
    ...
    "orgchart-print.html": {
      pageId: "orgchart-print",
      title: "Organization Chart (Print View)",
      path: "orgchart-print.html",
      section: "Main",
      summary: "Standalone print/PDF document view of the squadron Table of Organization.",
      primaryActions: ["Print", "Save as PDF"],
      forms: [],
      buttons: ["Print / Save as PDF", "Close"],
      relatedPages: ["orgchart.html"],
      keywords: ["print org chart", "org chart pdf", "table of organization", "print", "pdf"],
    },
    "flight-review.html": {
      pageId: "bfr",
    ...
    "resources.html": {
      pageId: "resources",
      title: "CAP References",
      path: "resources.html",
      section: "Main",
      summary: "Curated direct links to official Civil Air Patrol publications, regulations, forms, and operational pages, plus direct download buttons for squadron PowerPoint, memorandum (Word), and PDF templates.",
      primaryActions: ["Download squadron templates", "Open CAP publications", "Open eServices", "Open regulations"],
      forms: [],
      buttons: ["Download template", "Open link", "Open CAP search"],
      relatedPages: ["documents.html", "senior-member.html"],
      keywords: [
        "CAP",
        "references",
        "regulations",
    ...
    "create-profile.html": {
      ...
      section: "Account",
      summary: "Request portal access by filling out your TN-170 profile. Squadron leadership reviews each request before granting access.",
      primaryActions: ["Create Account"],
      forms: ["createProfileForm"],
      buttons: ["Create Account"],
      relatedPages: ["pending-approval.html", "login.html"],
      keywords: ["create account", "request access", "signup", "registration"],
      public: true,
    },
  ...
  const NAV_LINKS = [
    ...
    { label: "CAP References", target: "resources.html", section: "Main", keywords: ["CAP references", "publications", "regulations", "forms", "eServices"] },
  ...
  const KEYWORD_ROUTES = [
    ...
    { patterns: [/create\s+(a\s+)?meeting/i, /build\s+(the\s+)?schedule/i, /meeting\s+planner/i, /meeting\s+schedule/i, /create\s+(a\s+)?monthly\s+schedule/i, /edit\s+(the\s+)?monthly\s+schedule/i, /clone\s+(the\s+)?(previous\s+)?(month|schedule)/i, /new\s+monthly\s+schedule/i], target: "schedule.html", label: "Meetings" },
    ...
    { patterns: [/print\s+org\s*chart/i, /export\s+org\s*chart/i, /org\s*chart\s+pdf/i, /organization\s+chart\s+pdf/i, /table\s+of\s+organization\s+pdf/i, /print\s+organization\s+chart/i], target: "orgchart-print.html?autoprint=0", label: "Print Org Chart" },
    { patterns: [/org\s*chart/i, /organization\s+chart/i, /vacant\s+positions?/i, /edit\s+(the\s+)?org\s*chart/i, /build\s+(the\s+)?org\s*chart/i, /create\s+(an?\s+)?org\s*chart/i, /reset\s+(to\s+)?(tn[-\s]*170\s+)?default\s+(org\s*chart\s+)?structure/i, /clone\s+(an?\s+)?(existing\s+)?(org\s*chart|chart)/i, /table\s+of\s+organization/i], target: "orgchart.html", label: "Organization Chart" },
    { patterns: [/print\s+(monthly\s+)?schedule/i, /export\s+(monthly\s+)?schedule/i, /monthly\s+schedule\s+pdf/i, /meeting\s+schedule\s+pdf/i, /preview\s+print\s+schedule/i, /print\s+meetings?/i], target: "schedule-print.html?autoprint=0", label: "Print Monthly Schedule" },
    { patterns: [/resource\s+links?/i, /files?\s+(&|and)\s+resources?/i], target: "documents.html", label: "Files & Resources" },
    { patterns: [/cap\s+references?/i, /cap\.gov/i, /gocivilairpatrol/i, /eservices/i, /capnhq/i, /cap\s+publications?/i, /cap\s+regulations?/i, /cap\s+forms?/i, /\baxis\b/i], target: "resources.html", label: "CAP References" },
    { patterns: [/powerpoint\s+template/i, /\bpptx?\b/i, /memorandum\s+template/i, /memo\s+template/i, /word\s+template/i, /\bdocx?\b/i, /letterhead\s+template/i, /transportation\s+authorization/i, /business\s+letter/i, /squadron\s+templates?/i, /download\s+(a\s+)?template/i, /template\s+download/i], target: "resources.html", label: "Squadron Templates" },
