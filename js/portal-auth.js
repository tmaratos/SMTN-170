  ...
  async function guardPage() {
    ...
    if (loginOnlyPages.includes(path)) {
      if (hasAuthSession) {
        ...
        } else if (session && session.accountStatus === ACCOUNT_STATUS.AWAITING) {
          global.location.replace(PENDING_PATH);
        }
      }
      return;
    }
    ...
    if (!hasAuthSession) {
      ...
      session = null;
      profile = null;
      global.location.replace(LOGIN_PATH + "?expired=1");
      return;
    }
    ...
    if (path === PENDING_PATH) {
      if (session.accountStatus === ACCOUNT_STATUS.APPROVED) {
        global.location.replace("dashboard.html");
      }
      return;
    }
