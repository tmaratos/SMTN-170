import { IMPORT_TYPES, typeToTable } from "./import-meta.ts";
...
export function inferScheduleYear(text: string, fileName: string): number {
...
function inferContentMonth(text: string): number | null {
  const m = (text || "").match(
    /\b(january|february|march|april|may|june|july|august|september|october|november|december)\s+\d{4}\b/i,
  );
...
function summarizeBlockText(raw: string | undefined): string {
  return (raw || "")
...
export function isBctScheduleContent(text: string): boolean {
...
export function parseBctMeetingScheduleText(
  text: string,
  ...
  const weeks: { weekNum: number; month: number; day: number }[] = [];
  ...
  const rowPatterns: [string, RegExp][] = [
    ["uniform", /^uniform\b/i],
    ["opening", /^opening\b/i],
...
function parseDateToken(str: string): string | null {
  if (!str) return null;
...
export function scoreClassification(text: string, fileName: string): ClassificationResult {
  const content = text || "";
...
function parseOrgChartText(text: string): Record<string, unknown>[] {
...
function parseMeetingScheduleText(text: string, options: ParseOptions): Record<string, unknown>[] {
...
function parseInspectionText(text: string): Record<string, unknown>[] {
  const lines = parseCsvLines(text);
  const drafts: Record<string, unknown>[] = [];
  lines.forEach((line) => {
...
function parseFlightReviewText(text: string): Record<string, unknown>[] {
  const lines = parseCsvLines(text);
  const drafts: Record<string, unknown>[] = [];
  lines.forEach((line) => {
    const cols = line.includes(",")
...
function parseTasksText(text: string): Record<string, unknown>[] {
  const lines = parseCsvLines(text);
  const drafts: Record<string, unknown>[] = [];
  lines.forEach((line) => {
...
function parseRosterText(text: string, kind: string): Record<string, unknown>[] {
  const lines = parseCsvLines(text);
  return lines
...
export function parseDraftRecords(
  extractedText: string | null,
  detectedType: string,
  options: ParseOptions = {},
): { drafts: Record<string, unknown>[]; parsed: boolean; type: string | null } {
